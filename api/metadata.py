...
class Meta_Endpoints_API(remote.Service):
    ...
    def cohort_files(self, request):
        ...
        query = 'select SampleBarcode, DatafileNameKey, Pipeline, Platform, DataLevel, Datatype from metadata_data where SampleBarcode in (select sample_id from cohorts_samples where cohort_id=%s) and DatafileUploaded="true" '
        ...
        try:
            ...
            if cursor.rowcount > 0:
                for item in cursor.fetchall():
                    file_list.append(FileDetails(sample=item['SampleBarcode'], filename=item['DatafileNameKey'], pipeline=item['Pipeline'], platform=item['Platform'], datalevel=item['DataLevel'], datatype=item['Datatype']))
            else:
