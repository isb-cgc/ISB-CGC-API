...
def test_versions(client, app):
    ...
    assert versions['1.0']["data_sources"] == \
        ...
        {'data_type': 'Clinical, Biospecimen, and Mutation Data',
         ...
         {'data_type': 'Image Data', 'name': 'idc-dev-etl.idc_v1.dicom_pivot_v1'}]
    assert versions['2.0']['data_sources'] == \
           [{'data_type': 'Clinical, Biospecimen, and Mutation Data', 'name': 'isb-cgc.TCGA_bioclin_v0.Biospecimen'},
            ...
            {'data_type': 'Image Data', 'name': 'idc-dev-etl.idc_v2.dicom_pivot_v2'}]


def test_collections_v1(client, app):
...
def test_collections_v2(client, app):
...
def test_analysis_results_v2(client, app):
...
def test_attributes_v1(client, app):
    query_string = dict(
        idc_data_version = '1.0',
        data_source = 'idc-dev-etl.idc_v1.dicom_pivot_v1'
    )
    response = client.get('/v1/attributes',
                          query_string = query_string)
    assert response.status_code == 200
    assert response.json['idc_data_version'] == '1.0'
...
def test_attributes_v2(client, app):
    query_string = dict(
        idc_data_version = '2.0',
        data_source = 'idc-dev-etl.idc_v2.dicom_pivot_v2'
    )
    response = client.get('/v1/attributes',
                          query_string = query_string)
    assert response.status_code == 200
    assert response.json['idc_data_version'] == '2.0'
    data = response.json['data_sources']
    data_sources = {data_source['data_source']: {key: data_source[key] for key in data_source.keys() if key != 'data_source'} for data_source in data}
    attributes = {attribute['name']: {key: attribute[key] for key in attribute.keys() if key != 'name'} for attribute in data_sources[query_string['data_source']]['attributes']}
    assert 'Modality' in attributes
...
def test_attributes_all_data_sources_v1(client, app):
    query_string = dict(
        idc_data_version = '1.0',
        data_source = ''
    )
    ...
    for data_source in data_sources:
        ...
        if data_source == 'idc-dev-etl.idc_v1.dicom_pivot_v1':
            assert 'Modality' in attributes
            assert attributes['Modality'] == {'active': True, 'data_type': 'Categorical String', 'units': None}
...
def test_attributes_all_data_sources_v2(client, app):
    query_string = dict(
        idc_data_version = '',
        data_source = ''
    )
    ...
    for data_source in data_sources:
        attributes = {attribute['name']: {key: attribute[key] for key in attribute.keys() if key != 'name'} for attribute in data_sources[data_source]['attributes']}
        if data_source == 'idc-dev-etl.idc_v2.dicom_pivot_v2':
            assert 'Modality' in attributes
            assert attributes['Modality'] == {'active': True, 'data_type': 'Categorical String', 'units': None}
        elif data_source == 'isb-cgc.TCGA_bioclin_v0.Biospecimen':
            assert 'program_name' in attributes
